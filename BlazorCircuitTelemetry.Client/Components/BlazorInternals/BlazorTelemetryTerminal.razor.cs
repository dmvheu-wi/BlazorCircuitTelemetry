...
public partial class BlazorTelemetryTerminal
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _module = await JS.InvokeAsync<IJSObjectReference>("import", "./_content/BlazorCircuitTelemetry.Client/Components/BlazorInternals/BlazorTelemetryTerminal.razor.js");
        _self = DotNetObjectReference.Create(this);
        await _module.InvokeVoidAsync("start", _self);
        _observerStatus = "Observer active"; await InvokeAsync(StateHasChanged);
    }
    [JSInvokable] public Task ReceiveBatch(JsonElement[] batch) { foreach (var item in batch) { try { Add(item.Deserialize<TelemetryEntry>()!); } catch { _dropped++; } } return InvokeAsync(StateHasChanged); }
